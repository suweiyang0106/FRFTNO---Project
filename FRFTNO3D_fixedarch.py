...
class SpectralConv3d(nn.Module):
    def __init__(self, in_channels, out_channels, modes1, modes2, modes3, alpha=1.0): #suwei add alpha=1.0
        ...
        self.modes2 = modes2
        self.modes3 = 10#modes3#suwei change modes3 to 16
        if alpha == 1.0 :
            self.alpha = alpha
            self.alpha_flag=False
        else:
            self.alpha_flag=True
            # self.alpha = nn.Parameter(torch.tensor([1.0]))#nn.Parameter(2*torch.rand(1))
            self.alphax = nn.Parameter(2*torch.rand(1))#nn.Parameter(2*torch.rand(1))
            self.alphay = nn.Parameter(2*torch.rand(1))#nn.Parameter(2*torch.rand(1))
    ...
    def forward(self, x):
        ...
        if(self.alpha_flag==False):                                                    #suwei add
            x_ft = torch.fft.fftn(x, dim=[-3,-2,-1])
        else:
            scale = torch.sqrt(torch.tensor(x.shape[-1]))                       #suwei add
            x_ft = dfrft(x, self.alphax,dim=-3)#frft_shifted(x, self.alphax,dim=-3)*scale                    #suwei add
            x_ft = dfrft(x_ft, self.alphay,dim=-2)#frft_shifted(x_ft, self.alphay,dim=-2)*scale              #suwei add
            x_ft = torch.fft.fftn(x_ft, dim=[-1])#dfrft(x_ft, self.alpha,dim=-1)#frft_shifted(x_ft, self.alpha,dim=-1)*scale                #suwei add
        ...
        if(self.alpha_flag==False):
            ...
            x = dfrft(out_ft, -self.alphax,dim=-3)#frft_shifted(out_ft, -self.alphax,dim=-3)/scale               #suwei add
            x = dfrft(x, -self.alphay,dim=-2)#frft_shifted(x, -self.alphay,dim=-2)/scale                         #suwei add
            x = torch.fft.ifftn(x, s=(x.size(-1)))#dfrft(x, -self.alpha,dim=-1)#frft_shifted(x, -self.alpha,dim=-1)/scale                     # suwei add
        return x.real
...
class FNO3d(nn.Module):
    def __init__(self, modes1, modes2, modes3, width):
        ...
        self.conv0 = SpectralConv3d(self.width, self.width, self.modes1, self.modes2, self.modes3,1.5)
        self.conv1 = SpectralConv3d(self.width, self.width, self.modes1, self.modes2, self.modes3)
        self.conv2 = SpectralConv3d(self.width, self.width, self.modes1, self.modes2, self.modes3)
        self.conv3 = SpectralConv3d(self.width, self.width, self.modes1, self.modes2, self.modes3,1.5)
    ...
    def forward(self, x):
        ...
        x1 = self.conv0(x)
        x1 = self.mlp0(x1)   #suwei comment            
        x2 = self.w0(x)
        x = x1 + x2
        x = F.gelu(x)

        x1 = self.conv1(x)
        x1 = self.mlp1(x1)   #suwei comment        
        x2 = self.w1(x)
        x = x1 + x2
        x = F.gelu(x)

        x1 = self.conv2(x)
        x1 = self.mlp2(x1)   #suwei comment             
        x2 = self.w2(x)
        x = x1 + x2
        x = F.gelu(x)

        x1 = self.conv3(x)
        x1 = self.mlp3(x1)   #suwei comment             
        x2 = self.w3(x)
        x = x1 + x2       
...
ntrain = 30  #1000                               #suwei change 1000 to 4
ntest = 100
...
start, end = 0,1
train_a = a[start*ntrain:end*ntrain,:,:,:T_in]                                        #suwei add
train_u = a[start*ntrain:end*ntrain,:,:,T_in:T_in+T]                                  #suwei add
...
scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=iterations)#suwei comment
scheduler_step = 15#suwei add
scheduler_gamma = 0.5
